...
export default async function ArticlePage({ params }: PageProps) {
  ...
  return <main className="reading-page"><SiteHeader article />
    <header className="reading-hero"><StaticPageLink className="back-link" href="/serving-papers-atlas/#roadmap"><ArrowLeft />返回路线图</StaticPageLink><div className="reading-meta"><span>PAPER {article.no}</span><span>{article.source}</span><span>{article.date}</span><span>{article.readTime}</span><span>{article.difficulty}</span></div><h1>{article.shortTitle}</h1><p>{article.dek}</p><p className="original-title">ORIGINAL · {article.title}</p><div className="topology-strip"><Route /><span>{article.topology}</span></div></header>
    <nav className="section-rail" aria-label="文章章节">{article.sections.map((section,index)=><a key={section.title} href={`#section-${index+1}`}><span>0{index+1}</span>{section.title}</a>)}</nav>
    ...
    </article><aside className="reading-aside"><span>本章关键词</span><div>{article.tags.map(tag=><StaticPageLink key={tag} href="/serving-papers-atlas/#map">{tag}</StaticPageLink>)}</div><p><strong>阅读方法</strong>每读完一节，尝试用“瓶颈 → 状态所有权 → 通信 → SLO”复述一次。</p><section className="related-reading"><strong>关联阅读</strong>{article.related.map(slug=>{const item=getArticle(slug);return item?<StaticPageLink href={`/serving-papers-atlas/articles/${item.slug}/`} key={slug}>{item.no} · {item.shortTitle}</StaticPageLink>:null})}</section></aside></div>
    <nav className="paper-pagination">{previous?<StaticPageLink href={`/serving-papers-atlas/articles/${previous.slug}/`}><ArrowLeft /><span><small>上一篇 · {previous.no}</small>{previous.shortTitle}</span></StaticPageLink>:<span/>}{next?<StaticPageLink href={`/serving-papers-atlas/articles/${next.slug}/`}><span><small>下一篇 · {next.no}</small>{next.shortTitle}</span><ArrowRight /></StaticPageLink>:<StaticPageLink href="/serving-papers-atlas/#roadmap"><span><small>完成本轮</small>返回路线图</span><ArrowRight /></StaticPageLink>}</nav>
  </main>;
}
